	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (!ignore_offset && display_combined.y < resolution.y && display_combined.x < resolution.x)
		{
			...
			if (difference[0] > 1.0f)
			{
				...
				if (!m_scanmask_used || height_change > 4)
				{
			...
			if (!m_scanmask_used && display_diff.y < 4)
			{
		...
		else if(ignore_offset)// Stretch to fit the window
		{
			...
			if (!slbg || !feedback_merge)
			{
				...
				if (display_diff.y > 4 || (m_scanmask_used && display_diff.y == 1))
					off.y = display_diff.y;
			}
		...
		//DevCon.Warning("Offset x %d y %d rect x %d y %d z %d w %d", off.x, off.y, r.x, r.y, r.z, r.w);
